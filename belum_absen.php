...
$where_sub = "id NOT IN (SELECT siswa_id FROM absensi WHERE tanggal='$today' $pw)";
...
// Hitung "belum absen" per kelas untuk pill kelas
$belum_per_kelas = [];
foreach ($kelas_list as $k) {
    $k_safe = $conn->real_escape_string($k);
    $belum_per_kelas[$k] = (int)$conn->query(
        "SELECT COUNT(*) c FROM siswa WHERE kelas='$k_safe' AND id NOT IN (SELECT siswa_id FROM absensi WHERE tanggal='$today' $pw)"
    )->fetch_assoc()['c'];
}
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px">
            <?php foreach ($kelas_list as $k): $bk = $belum_per_kelas[$k]; $lengkap = $bk===0;
                $bg = $lengkap ? '#16a34a' : '#dc2626';
                $active = $kelas===$k;
            ?>
            <a href="?kelas=<?= urlencode($k) ?>"
               style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:999px;
                ...
                <i class="fas fa-<?= $lengkap ? 'check-circle' : 'exclamation-circle' ?>" style="font-size:.85rem"></i>
                KELAS <?= htmlspecialchars($k) ?> <?= $lengkap ? '(Lengkap)' : '('.$bk.' belum)' ?>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <table id="mainTable">
            ...
            <tbody>
                ...
                <tr data-id="<?= $row['id'] ?>"
                    ...
                    <td><?= $row['kelas'] ?></td>
                    <td>
                        <?php foreach (['Hadir'=>['success','#22c55e'],'Terlambat'=>['warning','#f59e0b'],'Sakit'=>['info','#06b6d4'],'Izin'=>['izin','#8b5cf6'],'Alpa'=>['secondary','#64748b'],'Bolos'=>['danger','#ef4444']] as $st=>$meta): ?>
                        <button onclick="absenCepat(<?= $row['id'] ?>, '<?= $row['nis'] ?>', '<?= addslashes($row['nama']) ?>', '<?= $row['kelas'] ?>', '<?= $st ?>')"
                            title="<?= $st ?>" style="margin:2px;border:none;border-radius:6px;padding:5px 12px;font-size:.78rem;font-weight:700;color:#fff;cursor:pointer;background:<?= $meta[1] ?>">
                            <?= $st ?>
                        </button>
                        <?php endforeach; ?>
                    </td>
...
<div id="bulkBar">
    ...
    <button class="btn-bulk terlambat" onclick="absenMassal('Terlambat')">⏰ Terlambat</button>
