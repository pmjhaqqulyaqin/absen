...
// Rekap per kelas untuk pill kelas (tercatat/total pada tanggal terpilih)
$rekap_per_kelas = [];
foreach ($kelas_list as $k) {
    $k_safe  = $conn->real_escape_string($k);
    $total_k = (int)$conn->query("SELECT COUNT(*) c FROM siswa WHERE kelas='$k_safe'")->fetch_assoc()['c'];
    $rec_k   = (int)$conn->query("SELECT COUNT(*) c FROM absensi WHERE tanggal='$tanggal' AND kelas='$k_safe'" . periode_where($conn))->fetch_assoc()['c'];
    $rekap_per_kelas[$k] = ['total' => $total_k, 'recorded' => $rec_k];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_batch'])) {
    ...
    foreach ($siswa_ids as $i => $sid) {
        $sid = (int)$sid;
        $st  = sanitize($statuses[$i] ?? 'Alpa');
        $ket = sanitize($keterangans[$i] ?? '');
        ...
        if ($jenis_absen === 'pulang') {
            ...
            $conn->query("INSERT INTO absensi (siswa_id,nis,nama,kelas,tanggal,jam_masuk,status,keterangan,metode,tahun_ajaran,semester)
                VALUES ($sid,'{$s['nis']}','{$s['nama']}','{$s['kelas']}','$tanggal_save',$jam,'$st','$ket','Manual',$pta,$psem)
                ON DUPLICATE KEY UPDATE status='$st',keterangan='$ket',metode='Manual'");
            $count++;
        }
    }
...
.btn-bulk.terlambat {background:#f59e0b;color:#fff}
.btn-bulk.sakit  {background:#06b6d4;color:#fff}
.btn-bulk.izin   {background:#8b5cf6;color:#fff}
...
.status-pick-btn {
    ...
    font-weight:700; color:#fff; opacity:.55; transition:all .15s;
}
.status-pick-btn:hover { opacity:.85; transform:scale(1.05); }
.status-pick-btn.active {
    opacity:1; border-color:#1e293b;
    box-shadow:0 0 0 2px rgba(30,41,59,.25); transform:scale(1.03);
}
</style>
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px">
            <?php foreach ($kelas_list as $k): $rk = $rekap_per_kelas[$k]; $done = $rk['total']>0 && $rk['recorded']>=$rk['total'];
                $bg = $done ? '#16a34a' : '#2563eb';
                $active = $kelas===$k;
            ?>
            <a href="?tanggal=<?= $tanggal ?>&kelas=<?= urlencode($k) ?>"
               style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:999px;
                ...
                <i class="fas fa-<?= $done ? 'check-circle' : 'edit' ?>" style="font-size:.85rem"></i>
                KELAS <?= htmlspecialchars($k) ?> <?= $done ? '(Lengkap)' : '('.$rk['recorded'].'/'.$rk['total'].')' ?>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="ms-auto" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
                ...
                <?php foreach (['Hadir'=>'#22c55e','Terlambat'=>'#f59e0b','Sakit'=>'#06b6d4','Izin'=>'#8b5cf6','Alpa'=>'#64748b','Bolos'=>'#ef4444'] as $s=>$c): ?>
...
<div id="bulkBar">
    ...
    <button class="btn-bulk terlambat" onclick="setMassal('Terlambat')">⏰ Terlambat</button>
...
const statuses  = ['Hadir','Terlambat','Sakit','Izin','Alpa','Bolos'];
