...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['aksi'])) {
    ...
    if ($aksi === 'pin_wali') {
        ...
        if (strlen($pin_wali) !== 4 || !ctype_digit($pin_wali)) {
            ...
            $stmt->bind_param("ssi", $hashed, $pin_wali, $wali_id);
            $stmt->execute(); $stmt->close();
    ...
    if ($aksi === 'pin_bk') {
        ...
        if (strlen($pin_bk) !== 4 || !ctype_digit($pin_bk)) {
            ...
            $stmt = $conn->prepare("UPDATE guru_bk SET pin=?, pin_plain=? WHERE id=?");
            $stmt->bind_param("ssi", $hashed, $pin_bk, $bk_id_t);
            $stmt->execute(); $stmt->close();
    ...
    if ($aksi === 'tambah_bk') {
        ...
        if ($nama_bk) {
            ...
            $pp = $conn->real_escape_string($pin_new);
            $conn->query("INSERT INTO guru_bk (nama,nip,pin,pin_plain) VALUES ('$nb','$np','$hashed','$pp')");
...
// Pastikan kolom pin_plain ada di wali (untuk menampilkan PIN apa adanya ke admin)
$chkp = $conn->query("SHOW COLUMNS FROM wali LIKE 'pin_plain'");
...
// Catatan: tabel guru_bk TIDAK di-seed otomatis lagi di sini.
// Dulu setiap kali tabel kosong sistem selalu memasukkan ulang 'Guru BK' default,
// akibatnya Guru BK yang dihapus selalu muncul lagi. Sekarang data dikelola murni
// lewat tombol Tambah BK / Hapus di bawah, jadi hapus benar-benar permanen.

// Pastikan kolom pin_plain ada di guru_bk (untuk menampilkan PIN apa adanya ke admin)
$chkpb = $conn->query("SHOW COLUMNS FROM guru_bk LIKE 'pin_plain'");
if ($chkpb && $chkpb->num_rows === 0) {
    $conn->query("ALTER TABLE guru_bk ADD COLUMN pin_plain VARCHAR(10) DEFAULT NULL");
}
// Ambil semua Guru BK
        ...
        <div class="col-lg-8">
            ...
            <div class="d-flex align-items-center mb-4">
                ...
                <div>
                    ...
                    <small class="text-muted">Atur PIN untuk login Admin dan Guru BK</small>
                </div>
            </div>
            ...
            <!-- PIN Kepala Sekolah dikelola sendiri oleh Kepsek lewat Portal Kepsek
                 (menu "PIN Kepala Sekolah ↗" / atur_pin_kepsek.php), jadi tidak
                 diduplikasi lagi di halaman admin ini. -->

            <!-- PIN WALI: disembunyikan (terlalu banyak / redundan & menampilkan Username
                 yang tidak diperlukan untuk login wali - login wali cukup pakai PIN).
                 Kode aslinya tetap disimpan di bawah, tinggal ubah "false" jadi "true"
                 kalau suatu saat ingin dimunculkan lagi. -->
            <?php if (false): ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($w['pin'])): ?>
                                        <span class="badge bg-success"><i class="fas fa-check-circle me-1"></i>PIN: <?= htmlspecialchars($w['pin_plain'] ?? '****') ?></span>
                                    <?php else: ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-success"><i class="fas fa-check-circle me-1"></i>PIN: <?= htmlspecialchars($bk['pin_plain'] ?? '****') ?></span></td>
                                <td>
...
<!-- Modal Set PIN Wali: disembunyikan bersamaan dengan kartu "PIN Wali Kelas" di atas -->
<?php if (false): ?>
