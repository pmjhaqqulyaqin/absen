...
// Tambah stat pulang
$stats['pulang'] = $conn->query("SELECT COUNT(*) c FROM absensi WHERE tanggal='$today' $pw AND jam_pulang IS NOT NULL")->fetch_assoc()['c'];
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $r = $conn->query("SELECT 
        ...
        FROM absensi WHERE tanggal='$d' $pw")->fetch_assoc();
    $chart_hadir[]     = (int)($r['hadir'] ?? 0);
    $chart_terlambat[] = (int)($r['terlambat'] ?? 0);
}

// Recent log
$recent = $conn->query("SELECT a.*, s.foto FROM absensi a LEFT JOIN siswa s ON s.id=a.siswa_id WHERE a.tanggal='$today' $pw ORDER BY a.updated_at DESC LIMIT 10");
