...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_log'])) {
    ...
    if (strtolower($konfirmasi) !== 'hapus') {
        ...
        $where_hapus = "tanggal BETWEEN '$dari' AND '$sampai'" . periode_where($conn);
...
if (isset($_GET['preview'])) {
    ...
    $where  = "tanggal BETWEEN '$dari' AND '$sampai'" . periode_where($conn);
...
// DATA RINGKASAN
// =============================================
$oldest    = $conn->query("SELECT MIN(tanggal) as v FROM absensi WHERE 1=1" . periode_where($conn))->fetch_assoc()['v'];
$newest    = $conn->query("SELECT MAX(tanggal) as v FROM absensi WHERE 1=1" . periode_where($conn))->fetch_assoc()['v'];
$total_log = $conn->query("SELECT COUNT(*) as c FROM absensi WHERE 1=1" . periode_where($conn))->fetch_assoc()['c'];
$total_hari= $conn->query("SELECT COUNT(DISTINCT tanggal) as c FROM absensi WHERE 1=1" . periode_where($conn))->fetch_assoc()['c'];
$total_siswa_log = $conn->query("SELECT COUNT(DISTINCT siswa_id) as c FROM absensi WHERE 1=1" . periode_where($conn))->fetch_assoc()['c'];
...
$per_bulan = $conn->query("SELECT YEAR(tanggal) as tahun, MONTH(tanggal) as bulan,
    ...
    FROM absensi WHERE 1=1" . periode_where($conn) . " GROUP BY YEAR(tanggal), MONTH(tanggal) ORDER BY tahun DESC, bulan DESC");
    ...
    <div class="table-container">
        <table>
            ...
            if (isset($_POST['hapus_backup'])) {
                ...
                $conn->query("DELETE FROM rekap_bulanan WHERE bulan=$hb AND tahun=$hy" . periode_where($conn));
            ...
            if (isset($_GET['download_backup'])) {
                ...
                $rows = $conn->query("SELECT nis, nama, kelas, hadir, terlambat, alpa, sakit, izin, bolos, total_hari FROM rekap_bulanan WHERE bulan=$db AND tahun=$dy" . periode_where($conn) . " ORDER BY kelas, nama");
            ...
            $rekap = $conn->query("SELECT bulan, tahun,
                ...
                FROM rekap_bulanan WHERE 1=1" . periode_where($conn) . " GROUP BY bulan, tahun ORDER BY tahun DESC, bulan DESC");
            if ($rekap->num_rows === 0): ?>
