...
$conn->query("ALTER TABLE siswa ADD COLUMN IF NOT EXISTS password_plain VARCHAR(255) DEFAULT NULL");
...
if ($action==='reset_pw' && isset($_GET['id'])) {
    ...
    if ($s) {
        ...
        $conn->query("UPDATE siswa SET password='$hash',password_plain='".$conn->real_escape_string($s['nis'])."' WHERE id=$id");
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_siswa'])) {
    ...
    $pw_sql = $pw_new ? ",password='".password_hash($pw_new,PASSWORD_DEFAULT)."',password_plain='".$conn->real_escape_string($pw_new)."'" : '';
    ...
    if ($id) {
        ...
        if ($chk) {
            ...
            $pw_plain_esc = $conn->real_escape_string($nis);
            $conn->query("INSERT INTO siswa (nis,nama,kelas,foto,password,password_plain,no_wa_ortu) VALUES ('$nis','$nama','$kelas',$foto_val,'$default_pw','$pw_plain_esc','$no_wa_ortu')");
...
// Jumlah siswa per kelas untuk pill kelas
$jumlah_per_kelas = [];
...
<div class="page-header d-flex align-center">
    ...
    <div class="ms-auto" style="display:flex;gap:8px">
        ...
        <a href="kelola_kelas.php" class="btn btn-primary"><i class="fas fa-people-arrows"></i> Kelola Kelas</a>
...
<div class="card mb-3"><div class="card-body">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px">
        <?php foreach ($kelas_list as $k): $active = $fkelas===$k; ?>
        <a href="?kelas=<?= urlencode($k) ?><?= $search ? '&search='.urlencode($search) : '' ?>"
           style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:999px;
                  ...
                  background:#2563eb;box-shadow:0 2px 6px rgba(0,0,0,.15);<?= $active ? 'outline:3px solid #1e3a8a;outline-offset:2px;' : '' ?>">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr id="row-<?= $s['id'] ?>">
                    ...
                    <td>
                        <code style="background:#f0fdf4;color:#15803d;padding:2px 6px;border-radius:4px;font-size:.8rem"
                              title="Password login siswa">
                            <?= !empty($s['password_plain']) ? htmlspecialchars($s['password_plain']) : '<i class="fas fa-key" style="color:var(--text-muted);opacity:.4"></i>' ?>
                        </code>
