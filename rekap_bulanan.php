...
$kelas_res  = $conn->query("SELECT nama_kelas FROM kelas ORDER BY nama_kelas");
// Fallback: ambil dari data siswa jika tabel kelas kosong
if ($kelas_res->num_rows === 0) {
    $kelas_res = $conn->query("SELECT DISTINCT kelas AS nama_kelas FROM siswa ORDER BY kelas");
}
$kelas_list = [];
while ($kr = $kelas_res->fetch_assoc()) $kelas_list[] = $kr['nama_kelas'];
...
<div class="card mb-3 no-print">
    <div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px">
            <?php foreach ($kelas_list as $k): $active = $kelas===$k; ?>
            <a href="?bulan=<?= $bulan ?>&tahun=<?= $tahun ?>&kelas=<?= urlencode($k) ?>"
