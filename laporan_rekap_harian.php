...
foreach ($kelas_list as $k) {
    ...
    $stats_q = $conn->query("SELECT status, COUNT(*) total
        FROM absensi
        WHERE tanggal='$tanggal_full' AND kelas='$k_safe'" . periode_where($conn) . "
        GROUP BY status");
...
if (!in_array($filter_kelas, $kelas_list, true)) $filter_kelas = "";
$kelas_sql     = $filter_kelas !== "" ? "AND a.kelas='".$conn->real_escape_string($filter_kelas)."'" : "";

$rekap_semua_q = $conn->query("SELECT a.nis, a.nama, a.kelas, a.status, a.keterangan FROM absensi a WHERE a.tanggal='$tanggal_full' ".$status_sql." ".$kelas_sql.periode_where($conn, 'a.')." ORDER BY a.status, a.kelas, a.nama");
$rekap_semua   = $rekap_semua_q ? $rekap_semua_q->fetch_all(MYSQLI_ASSOC) : [];

// Export Excel
if (isset($_GET["export_excel"])) {
    header("Content-Type: application/vnd.ms-excel; charset=utf-8");
    $fn = "Rekap_".$tanggal_full.($filter_status?"_".$filter_status:"_Semua").($filter_kelas?"_".$filter_kelas:"").".xls";
    ...
    echo "<tr><th colspan='6' style='text-align:center'>REKAP ABSENSI HARIAN - ".strtoupper($hari_nama).", ".$tgl." ".$nama_bulan[$bln]." ".$thn.($filter_status?" - STATUS: ".$filter_status:" - SEMUA STATUS").($filter_kelas?" - KELAS: ".$filter_kelas:"")."</th></tr>";
    ...
    <div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:14px">
            <?php foreach ($rekap_kelas as $r):
                $belum = $r['siswa'] - ($r['Hadir']+$r['Terlambat']+$r['Alpa']+$r['Sakit']+$r['Izin']+$r['Bolos']);
                $done  = $r['siswa'] > 0 && $belum <= 0;
                $bg = $done ? '#16a34a' : '#dc2626';
                $active = $filter_kelas===$r['kelas'];
            ?>
            <a href="?tgl=<?= $tgl ?>&bulan=<?= $bln ?>&tahun=<?= $thn ?>&kelas=<?= urlencode($r['kelas']) ?>#rekap-semua"
               style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:999px;
                      ...
                      background:<?= $bg ?>;box-shadow:0 2px 6px rgba(0,0,0,.15);<?= $active ? 'outline:3px solid #1e3a8a;outline-offset:2px;' : '' ?>">
                <i class="fas fa-<?= $done ? 'check-circle' : 'exclamation-circle' ?>" style="font-size:.85rem"></i>
                KELAS <?= htmlspecialchars($r['kelas']) ?> <?= $done ? '(Lengkap)' : '('.max($belum,0).' belum)' ?>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr style="background:<?= $row_bg ?>;transition:background .15s"
                    ...
                    <td style="padding:10px 12px;font-weight:700;font-size:.95rem">
                        <a href="?tgl=<?= $tgl ?>&bulan=<?= $bln ?>&tahun=<?= $thn ?>&filter_status=<?= urlencode($filter_status) ?>&kelas=<?= urlencode($r['kelas']) ?>#rekap-semua"
                           style="color:#1e3a8a;text-decoration:none" title="Lihat detail kelas <?= htmlspecialchars($r['kelas']) ?>">
...
<!-- ====== REKAP SEMUA STATUS ====== -->
<div id="rekap-semua" style="margin-top:28px;scroll-margin-top:20px">
    <!-- Header + Filter -->
    <div style="background:linear-gradient(135deg,#1e3a8a,#1d4ed8);color:white;border-radius:12px 12px 0 0;padding:14px 20px;display:flex;align-items:center;flex-wrap:wrap;gap:10px">
        <div style="font-weight:700;font-size:1rem;flex:1;min-width:200px">
            ...
            <span style="font-size:.78rem;font-weight:400;margin-left:8px;opacity:.85">— <?= $filter_kelas !== '' ? 'Kelas '.htmlspecialchars($filter_kelas) : 'Semua Kelas' ?></span>
        </div>
        <form method="GET" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <?php if ($filter_kelas !== ''): ?>
            <span style="background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.35);border-radius:20px;padding:5px 6px 5px 12px;font-size:.82rem;font-weight:600;display:flex;align-items:center;gap:6px;white-space:nowrap">
                <i class="fas fa-door-open"></i> <?= htmlspecialchars($filter_kelas) ?>
                <a href="?tgl=<?= $tgl ?>&bulan=<?= $bln ?>&tahun=<?= $thn ?>&filter_status=<?= urlencode($filter_status) ?>#rekap-semua"
                   title="Hapus filter kelas"
                   style="color:white;text-decoration:none;background:rgba(255,255,255,.25);border-radius:50%;width:18px;height:18px;display:flex;align-items:center;justify-content:center;font-size:.7rem;line-height:1">
                    <i class="fas fa-times"></i>
                </a>
            </span>
            <?php endif; ?>
            <a href="?tgl=<?= $tgl ?>&bulan=<?= $bln ?>&tahun=<?= $thn ?>&filter_status=<?= urlencode($filter_status) ?>&kelas=<?= urlencode($filter_kelas) ?>&export_excel=1"
        ...
        <table id="rekapSemuaTable" style="width:100%;border-collapse:collapse;font-size:.875rem">
            ...
            <tr style="background:#1e3a8a;color:white;font-weight:800">
                ...
                <td colspan="4" style="padding:10px 12px"><?= count($rekap_semua) ?> siswa
                    ...
                    <?= $filter_kelas !== '' ? " — Kelas: <strong>".htmlspecialchars($filter_kelas)."</strong>" : "" ?>
                </td>
            </tr>
            <?php endif; ?>
            </tbody>
